  ...
  <md-list class="user-list">
    <md-toolbar class="md-transparent" md-elevation="0">
      <div class="user-list__header">
        <span class="user_list__title">Users connected</span>
        <span class="user-list__status">
          <md-icon @click.native="changeStatus()" v-bind:class="myStatus"
  ...
  methods: {
    ...
    changeStatus() {
      this.$store.dispatch(STORE_ACTIONS.changeStatus).then(() => {
        this.$sockert.emit(WS_EVENTS.changeStatus, this.$store.state);
      });
    },
  },
  computed: {
    myStatus() {
      return {
        avaialable: this.$store.state.status === STATUS_OPTIONS.available,
...
.md-toolbar.md-icon.md-theme-default.md-transparent {
    background: #486ca9;
    color: white;
    font-size: 17px;
    position: sticky;
